...
describe("/api/ideas routes", function () {
  ...
  describe("PUT /api/ideas/:ideaId", function () {
    ...
    // fixed from original codecademy file as test failed due to a different error being caught
    // before the onvalid ID issue because no other valid data was being sent
    it("called with an invalid ID returns a 404 error", function () {
      return request(app)
        .get("/api/ideas/1")
        .then((response) => {
